...
#     For actual sophisticated template integration (e.g., filling placeholders in a
#     DOCX template, preserving its layout, or rendering complex HTML/CSS templates),
#     you would need more advanced libraries or external tools.
...
#     # Define custom styles for resume elements
...
#     story.append(Paragraph("--- Main Resume Content ---", style_h2)) # Separator

#     # Add the main edited resume text (now AI-generated)
#     # This part assumes the AI generates semi-structured text with lines/paragraphs.
#     # For true section-based templates, you'd parse AI text into sections (e.g., Experience, Education)
#     # and map them to template placeholders.
#     lines = edited_text.split('\n')
#     for line in lines:
#         stripped_line = line.strip()
#         if stripped_line.startswith('- '): # Simple bullet point detection
#             story.append(Paragraph(stripped_line.lstrip('- ').strip(), style_bullet))
#         elif stripped_line:
#             story.append(Paragraph(stripped_line, style_normal))
#         else:
#             story.append(Spacer(1, 0.1 * inch)) # Add some space for blank lines
...
#         traceback.print_exc()
#         # Fallback to a very basic PDF if template rendering fails
...
#         p_fallback.drawString(inch, letter[1] - inch - 60, edited_text[:500] + "...") # Show first 500 chars
#         p_fallback.save()
...
#         # Prompt for resume analysis - MODIFIED TO REQUEST FULL REVISED RESUME TEXT
#         analysis_prompt = f"""
...
# ```
# """
#         # Define JSON schema for structured response - MODIFIED
#         analysis_schema = {
#             "type": "OBJECT",
#             "properties": {
...
#             'extracted_resume_text': resume_text, # Still send original extracted text for reference/debugging
#             'ai_revised_full_resume_text': analysis_result.get('ai_revised_full_resume_text', '') # NEW field
#         }, room=sid)
...
#     edited_resume_text = data.get('edited_resume_text') # This now contains the AI-generated text
...
#         await sio.emit('error', {'message': 'Please select a resume template.'}, room=sid)
#         return

#     try:
#         # Pass both edited_text (which is the AI-generated text) and revised_summary to the template function
